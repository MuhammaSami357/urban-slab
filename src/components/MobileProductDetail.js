...
import MobileProductData from '../data/MobileProductData';
const MobileProductDetail = () => {
    ...
    const { id } = useParams();
    const MobileProduct = MobileProductData.find((item) => item.id === id);
  ...
  return (
    <div className="product-detail">
      <img src={MobileProduct.image} alt={MobileProduct.name} className="product-image" />
      <div className="product-info">
        <h2>{MobileProduct.name}</h2>
        <p className="price">{MobileProduct.price}</p>
        <p className="desc">
